				...
				<?php if ( count($rows) ): ?>
					...
					<?php foreach ($rows as $row): ?>
						<tr>
							...
							<td>
								<?php echo $row->author ? CHtml::encode(urldecode($row->author->username)) : Yii::t('adminglobal', 'Unknown'); ?>
								<?php if( $row->last_edited_author ): ?>
									<br /><small><span class='tooltip' title='<?php echo Yii::t('admincustompages', 'Last Modified By'); ?>'><?php echo $row->lastauthor ? CHtml::encode(urldecode($row->lastauthor->username)) : Yii::t('adminglobal', 'Unknown'); ?></span></small>
								<?php endif; ?>
							</td>
							<td>
							<?php if( $row->language ): ?>
							...
							<td>
								<!-- Icons -->
								 <?php /* <a href="<?php echo Yii::app()->createAbsoluteUrl('/' . $row->alias, array('lang'=>$row->language) ); ?>" title="<?php echo Yii::t('admincustompages', 'view this page'); ?>" target='_blank' class='tooltip'><img src="<?php echo Yii::app()->themeManager->baseUrl; ?>/images/icons/preview.png" alt="Preview" /></a> */ ?>
