    ...
    'htmlOptions' =>array('class' => 'my-grid-view'),
	 'columns'=>array(
         ...
         array(
             ...
             'filter' => $model->getDropNames('menu_id'),
             'visible' => !isset($_GET['menu_id']),
         ),
         array(
             'name' => 'static_page_id',
             'value' => array($this,'renderCustomPageCol'),
             'filter' => CHtml::activeDropDownList($model, 'static_page_id',
                 CHtml::listData(CustomPages::model()->findAll(
                 array('select' => 'title, id',
                     'order' => 'title',
                 ))
                , "id", "title"), array('id' => false, 'prompt' => '')),
             'type' => 'raw',
         ),
         //'link',
         array(
