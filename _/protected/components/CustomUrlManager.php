...
class CustomUrlManager extends CUrlManager {
    ...
    protected function processRules() {
        ...
        if( ($urlrules = Yii::app()->cache->get('customurlrules')) === false || true)
		{
            ...
            if(Yii::app()->params['single_language'])
            {
                foreach($urlRules as $rule)
                {
                    $_more[ "page/<alias:({$rule['alias']})>" ] = array('site/custompages/index');
                }
                $this->rules = array(
                    'profile/' => 'profile/my',
                    'admin' => 'admin/index',
                    'startup/index' => 'startup/index',
                    'startup/<action:\w+>/<id:\d+>'=>'startup/<action>',
                    'startup/view/<alias:\w+>'=>'startup/view',
                    //'startup/new'=>'startup/team/8',
                    //'startup/<alias:(.*)>'=>'startup/view',
                    'info/<alias:(.*)>'=>'info/index',
                    'signature/<username:(.*)>'=>'signature/view',
                    'profile/<username:(.*)>'=>'profile/view',
                    '<controller:\w+>/<id:\d+>'=>'<controller>/view',
                    '<controller:\w+>/<id:\d+>/<name:\w+>'=>'<controller>/view',
                    '<controller:\w+>/<action:\w+>/<id:\d+>'=>'<controller>/<action>',
                    '<controller:\w+>/<action:\w+>'=>'<controller>/<action>',
                    /*
                                          // Match by index/index
                                        "admin" => array('admin/index/index'),

                                        // Match by controller/index
                                        "admin/<_c:([a-zA-z0-9-]+)>" => array('admin/<_c>/index/'),

                                        // Match by controller/action/*
                                        "admin/<_c:([a-zA-z0-9-_]+)>/<_a:([a-zA-z0-9-_]+)>/*" => array('admin/<_c>/<_a>/'),
                                        ...
                                        // User profile
                                        "profile/home" => array('site/users/home'),
                                        "profile/<uid:(\d+)>-<alias:(.*)>" => array('site/users/viewprofile'),

                                        // Match by controller/index
                                        "<_c:([a-zA-z0-9-]+)>" => array('site/<_c>/index'),
                                        // Match by controller/id/* => view
                                        "<_c:([a-zA-z0-9-_]+)>/<id:(\d+)>/*" => array('site/<_c>/view/'),

                                        // Match by controller/action/*
                                        "<_c:([a-zA-z0-9-_]+)>/<_a:([a-zA-z0-9-_]+)>/*" => array('site/<_c>/<_a>/'),
                                        /*
                                        // Match by controller/action/* and more
                                        "<_c:([a-zA-z0-9-_]+)>/<_a:([a-zA-z0-9-_]+)>/*" => array('site/<_c>/<_a>'),

                                        // Match by index/index
                                        "<lang:({$active_lang})>" => array('site/index/index'),
                                        "" => array('site/index/index'),

                                        // Match by controller index
                                        "<_c:([a-zA-z0-9-_]+)>/*" => array('site/<_c>/index/'),
                                        "<_c:([a-zA-z0-9-_]+)>/*" => array('site/<_c>/index'),
                    */                );
            }
            else
			...
			//$urlrules = array_merge(  $this->rules , $_more);
			$urlrules = $this->rules ;
    ...
    public function createUrl($route,$params=array(),$ampersand='&')
    {
        ...
        if ( count( Yii::app()->params['languages'] ) > 1 &&  Yii::app()->params['single_language'] == false)
        {
