...
class userAutoComplete extends CJuiAutoComplete
{
...
//CHtml::hiddenField($name,$this->value,$this->htmlOptions);          
        ...
        */
        ...
        if (!isset($this->options['success'])) {
            $this->options['success'] = 'js:function(event, ui) {
            $("#'.$id.'").val(ui.item.message);
            $("#'.$attr_id.'").val(ui.item.id);
        }';
        }
    ...
    {
        return '.data( "autocomplete" )._renderItem = function( ul, item ) {
        return $( "<li>" )
            .data( "item.autocomplete", item )
            .append( "<a class=\'aoutoSearchBox\'><div class=\'autoName\'>" + item.label + "</div><img class=\'userPicSmall\' src=" + item.fbuid + "></a>" )
            .appendTo( ul );
            };';
    }
